...
public class PayWithdrawalConfigDao {
    ...
    private static final String PAY_CONFIG = "select * from pay_withdrawal_config where enabled = 1 limit 1;";
    ...
    private static final String FIND_BY_CHANNEL = "select * from pay_withdrawal_config where enabled = 1 and channel = ";
    ...
    private static final String FIND_BY_NAME = "select * from pay_withdrawal_config where name = '";
    public PayWithdrawalConfig findByName(String name) {
        String sql = FIND_BY_NAME + name + "';";
        return (PayWithdrawalConfig) JdbcOrm.getInstance().getBean(sql, PayWithdrawalConfig.class);
    }

    private static final String ENABLE_PAY_CONFIG = "select * from pay_withdrawal_config where enabled = 1;";
    ...
    public PayWithdrawalConfig findById(int id) {
        String sql = "select * from pay_withdrawal_config where id = " + id + ";";
        return (PayWithdrawalConfig) JdbcOrm.getInstance().getBean(sql, PayWithdrawalConfig.class);
    }
}
