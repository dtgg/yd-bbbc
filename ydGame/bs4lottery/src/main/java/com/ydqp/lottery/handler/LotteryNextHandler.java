...
@ServerHandler(module = "lottery", command = 5000007)
public class LotteryNextHandler implements IServerHandler {

    private static final Logger logger = LoggerFactory.getLogger(LotteryNextHandler.class);

    @Override
    public void process(ISession iSession, AbstartParaseMessage abstartParaseMessage) {
        LotteryNext lotteryNext = (LotteryNext)abstartParaseMessage;
        PlayerData playerData = PlayerCache.getInstance().getPlayer(lotteryNext.getConnId());
        if (playerData == null) {
        ...
        LotteryNextSuc suc = new LotteryNextSuc();
        suc.setLotteryTypeInfos(infos);
        iSession.sendMessageByID(suc, abstartParaseMessage.getConnId());
        logger.info("{}获取了下期数据, connId:{}", playerData.getPlayerId(), lotteryNext.getConnId());
    }
}
